...
> 
> Option 2: In ***ArcGIS Pro***, update the layer sources using the `Update Data Sources` tool in the `Catalog` view
...
Currently, WFS layers are not supported, but SLYR will create an empty layer for them. 
...
>
>  - In the `Browser panel`, click on the `WFS` group and add in the layer's name and url as a `New Connection`.
>
>  ![WFS New Connection](../images/unavailable-layers-4.png)
> 
>  - In the `Layer Panel`, right-mouse-click on the layer and select `Repair Data Source` 
>
