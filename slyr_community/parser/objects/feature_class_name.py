...
class FeatureClassName(Object):
    ...
    def from_dict(cls, definition: dict) -> "FeatureClassName":
        ...
        res.feature_type = [
            k
            for k, v in FeatureClassName.FEATURE_TYPES.items()
            if v == definition["feature_type"]
        ][0]
...
class CoverageFeatureClassName(FeatureClassName):
    ...
    def to_dict(self):  # pylint: disable=method-hidden
        res = super().to_dict()
        res["class_type"] = CoverageFeatureClassName.FEATURE_CLASS_TYPES[
            self.class_type
        ]
    ...
    def from_dict(cls, definition: dict) -> "CoverageFeatureClassName":
        ...
        res.class_type = [
            k
            for k, v in CoverageFeatureClassName.FEATURE_CLASS_TYPES.items()
            if v == definition["class_type"]
        ][0]
        res.topology = [
            k
            for k, v in CoverageFeatureClassName.FEATURE_CLASS_TOPOLOGY.items()
            if v == definition["topology"]
        ][0]
