...
class DiagramConverter:
    ...
    @staticmethod
    def convert_diagrams(renderer: ChartRenderer,
        ...
        if isinstance(chart_symbol_layer, (BarChartSymbol, StackedChartSymbol)):
            ...
            if isinstance(chart_symbol_layer, (BarChartSymbol, )):
                try:
                    if chart_symbol_layer.axis_symbol:
                        axis_symbol = SymbolConverter.Symbol_to_QgsSymbol(chart_symbol_layer.axis_symbol, context)
            ...
            if isinstance(chart_symbol_layer, (BarChartSymbol, )):
                if chart_symbol_layer.spacing:
                    try:
                        settings.setSpacing(context.convert_size(chart_symbol_layer.spacing))
            ...
            if isinstance(chart_symbol_layer, (StackedChartSymbol, )):
                if Qgis.QGIS_VERSION_INT >= 31200:
                    ...
                    if chart_symbol_layer.fixed_length:
                        ...
                        if isinstance(chart_symbol_layer, ):
                            settings.size = QSizeF(context.convert_size(chart_symbol_layer.size),
                                                   context.convert_size(chart_symbol_layer.size))
                        else:
                            settings.size = QSizeF(context.convert_size(chart_symbol_layer.max_length),
                                                   context.convert_size(chart_symbol_layer.max_length))
                        settings.sizeType = context.units
                    else:
                        expression = '+'.join([QgsExpression.quotedColumnRef(c) for c in renderer.attributes])
                        ...
                        if isinstance(chart_symbol_layer, ):
                            ...
                            diagram_renderer.setUpperSize(
                                QSizeF(context.convert_size(chart_symbol_layer.size or 0), context.convert_size(chart_symbol_layer.size or 0)))
                        else:
                            diagram_renderer.setUpperValue(renderer.symbol.max_value)
                            diagram_renderer.setUpperSize(
                                QSizeF(context.convert_size(renderer.symbol.max_length), context.convert_size(renderer.symbol.max_length)))
                        settings.sizeType = context.units
                    ...
                    if context.unsupported_object_callback:
                        ...
                        context.unsupported_object_callback(
                            'Stacked charts require QGIS 3.12 or later', level=Context.WARNING)
            ...
            if chart_symbol_layer.outline:
                outline = SymbolConverter.Symbol_to_QgsSymbol(chart_symbol_layer.outline, context)
            ...
            raise NotImplementedException(
                'Converting {} diagrams is not yet supported'.format(chart_symbol_layer.__class__.__name__))
